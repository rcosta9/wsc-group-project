...
use LogicException;
use VendorFPF\Monolog\Handler\NullHandler;
use VendorFPF\Monolog\Logger;
use VendorFPF\Monolog\Registry;
use VendorFPF\Monolog\Handler\StreamHandler;
use Psr\Log\LogLevel;
use VendorFPF\WPDesk\Logger\WC\WooCommerceCapture;
use VendorFPF\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \VendorFPF\WPDesk\Logger\BasicLoggerFactory
{
    ...
    public static function tearDown($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (\VendorFPF\Monolog\Registry::hasLogger($name)) {
            \VendorFPF\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\VendorFPF\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \VendorFPF\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \VendorFPF\Monolog\Logger($name);
        }
        if (\VendorFPF\Monolog\Registry::hasLogger($name)) {
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \VendorFPF\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\VendorFPF\Monolog\Logger $logger)
    {
        if (!\defined('VendorFPF\\WC_LOG_THRESHOLD')) {
            \define('VendorFPF\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \VendorFPF\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \VendorFPF\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \VendorFPF\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \VendorFPF\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\VendorFPF\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \VendorFPF\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \VendorFPF\Monolog\Registry::hasLogger($name);
    }
}
