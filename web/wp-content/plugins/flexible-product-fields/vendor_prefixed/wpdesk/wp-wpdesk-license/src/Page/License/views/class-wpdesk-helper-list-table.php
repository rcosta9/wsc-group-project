...
class WPDesk_Helper_List_Table extends \WP_List_Table
{
    ...
    {
        echo \wpautop(\__('No WP Desk plugins found.', 'flexible-product-fields'));
    }
    ...
    {
        $columns = ['product_name' => \__('Plugin', 'flexible-product-fields'), 'product_status' => \__('Subscription Status', 'flexible-product-fields'), 'product_license' => \__('Subscription Data', 'flexible-product-fields')];
        return $columns;
    }
    ...
    {
        $status = \__('Deactivated', 'flexible-product-fields');
        if ($item['activation_status'] == 'Activated') {
            $status = \__('Activated', 'flexible-product-fields');
        }
        return $status;
    }
